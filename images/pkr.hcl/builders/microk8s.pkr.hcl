...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "fi",
      "",
      "# Setup tmux isolation directory if AXIOM_INSTANCE_NAME is set",
      "if [ -n \"$AXIOM_INSTANCE_NAME\" ]; then",
      "    echo 'Setting up tmux isolation directory...'",
      "    mkdir -p \"/tmp/tmux-$AXIOM_INSTANCE_NAME\"",
      "    chown op:op \"/tmp/tmux-$AXIOM_INSTANCE_NAME\"",
      "    chmod 755 \"/tmp/tmux-$AXIOM_INSTANCE_NAME\"",
      "    echo \"Tmux isolation directory created: /tmp/tmux-$AXIOM_INSTANCE_NAME\"",
      "fi",
      "",
